...
python_cmd=(
  ...
  --output-dir /home/wdz/BT/MoE/lora-moe/outputs/vision_weather_lora_qv_v2                  # LoRA adapter、projector 和训练参数输出目录
  --device-map auto                                                                         # Qwen 多卡切分方式
  --dtype bfloat16                                                                          # Qwen 权重精度
  --batch-size 1                                                                            # 每步样本数
  --grad-accum-steps 16                                                                     # 梯度累积步数，有效 batch=batch_size*grad_accum_steps
  --epochs 5                                                                                # 训练轮数
  ...
  --save-steps 100                                                                          # 每隔多少优化步保存 checkpoint，0 表示只保存最终产物
  --eval-steps 100                                                                          # 每隔多少优化步跑验证，0 表示不中途验证
  ...
  --lora-r 16                                                                                # LoRA rank
  --lora-alpha 32                                                                           # LoRA alpha，通常为 2*r
